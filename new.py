...
import os, subprocess, re, html.parser, inspect, sys, MARC_lang, platform
...
class utilityFunctions:
    ...
    def listChangeScripts(self, BatchEdits):
        ...
        for i in dir(BatchEdits)[:-2]:
            print(i)
            num = num + 1
            print(str(inspect.getargspec(getattr(BatchEdits, i))[3][0]))
            ChangeScriptsDict[num] = [i, str(inspect.getargspec(getattr(BatchEdits, i))[3])]
            #ChangeScriptsDict[num] = [i, ''.join(inspect.getargspec(getattr(BatchEdits, i))[3])]
        for key in ChangeScriptsDict.keys():
            print(key, ': ' + ChangeScriptsDict[key])
